# ===== database.py (FULLY UPDATED - ALL HTML FIXES) =====
...
class Database:
    ...
    def _create_indexes(self):
        try:
            ...
            # UPDATED: missions per user per date per mission_id
            self.missions.create_index([('user_id', ASCENDING), ('date', ASCENDING), ('mission_id', ASCENDING)], unique=True)
            # UPDATED: daily_claims — permanent (no date), user_id + ad_id unique
        ...
        try:
            if self.ads.count_documents({}) == 0:
                self.ads.insert_many([
                    {'id': 1, 'title': 'Install App & Earn', 'reward': 2.0, 'link': 'https://t.me/+8SdeM5gBihoxZjU1', 'meta': '⏱️ 2 min • 1.2k completed', 'icon': '📱', 'order': 1, 'edited_at': None, 'claim_code': None},
                    {'id': 2, 'title': 'Watch Video', 'reward': 0.5, 'link': 'https://t.me/+8SdeM5gBihoxZjU1', 'meta': '⏱️ 30 sec • 3.4k completed', 'icon': '🎬', 'order': 2, 'edited_at': None, 'claim_code': None},
                    {'id': 3, 'title': 'Join Channel', 'reward': 1.0, 'link': 'https://t.me/+8SdeM5gBihoxZjU1', 'meta': '⏱️ 1 min • 5.6k completed', 'icon': '📢', 'order': 3, 'edited_at': None, 'claim_code': None}
                ])
    ...
    def add_live_activity(self, activity_type, user_id, amount=0, description="", extra=None):
        try:
            ...
            # UPDATED: store extra info like referred_name, referrer_name
            if extra:
        ...
        try:
            ...
            for act in activities:
                ...
                result.append({
                    ...
                    # UPDATED: pass referral info for live activity display
    ...
    def activate_referral_by_log_channel(self, referred_id):
        ...
        try:
            ...
            # UPDATED: live activity with referred name info
    ...
    def record_daily_search(self, referred_user_id):
        ...
        try:
            ...
            # Check already credited today for this referred user
            ...
            # Update referral last_search_date
            self.referrals.update_one(
            ...
            # UPDATED: live activity with referrer info
            self.add_live_activity(
    ...
    # ========== DAILY REFERRAL EARNINGS (search-based) ==========

    def process_daily_referral_earnings(self):
        """
        Daily earnings — only for referred users who searched today.
        Called at midnight via scheduled job.
        """
        if not self.ensure_connection():
            return 0
        try:
            today = datetime.now().date().isoformat()
            # Find all searches done today
            ...
            for search in today_searches:
                try:
                    ...
                    if referrer and not referrer.get('withdrawal_blocked') and not referrer.get('suspicious_activity'):
                        # Already credited in record_daily_search, just count
                        earnings_count += 1
                except Exception as e:
    ...
    def claim_day_bonus(self, user_id, date_str):
        try:
            ...
            # UPDATED: streak breaks if yesterday not claimed
            if last_daily:
                try:
    ...
    # ========== MISSIONS (FULLY UPDATED — each mission separate) ==========
    ...
    def get_user_missions(self, user_id):
        """Get all missions for user today — each one separate."""
        try:
            user_id = int(user_id)
            today = datetime.now().date().isoformat()
            result = {}
            ...
            for mdef in self.MISSIONS_DEF:
                ...
                if not doc:
                    # Auto-fill progress from user data
                    progress = 0
    ...
    def _update_single_mission_progress(self, user_id, mission_id, count=1):
        """Internal helper to update one mission's progress."""
        try:
            user_id = int(user_id)
            today = datetime.now().date().isoformat()
    ...
    def claim_single_mission(self, user_id, mission_id, reward):
        """Claim one specific mission reward."""
        try:
            user_id = int(user_id)
            today = datetime.now().date().isoformat()
    ...
    # ========== ADS (PERMANENT CLAIM — no date, reset on edit) ==========
    ...
    def update_ad(self, ad_id, title, reward, link, meta, icon=None, claim_code=None):
    ...
    def get_user_claimed_ads(self, user_id):
        """UPDATED: permanent claim check — no date filter."""
        try:
            user_id = int(user_id)
    ...
    def claim_ad(self, user_id, ad_id, reward):
        """UPDATED: permanent claim — user can only claim once ever (until admin edits)."""
        try:
            user_id = int(user_id)

            # Permanent check — no date
    ...
    def reset_ad_claims(self, ad_id):
        """Reset all claims for an ad (called when admin edits)."""
        try:
            self.daily_claims.delete_many({'ad_id': ad_id})
    ...
    # ========== LEADERBOARD (UPDATED — show all, prize only for 50/200+) ==========
    ...
    # ========== GAME FUNCTIONS (NO DAILY LIMITS — pass based only) ==========
    ...
    # 0.10 = ~60%, 0.50 = ~25%, 1.0 = ~10%, 2.0 = ~5%
    SPIN_WEIGHTS = [15, 6, 15, 3, 6, 15, 1, 15]

    def get_game_state(self, user_id, date=None):
        """UPDATED: no daily limits — only pass-based."""
        try:
            user_id = int(user_id)
            if not date:
    ...
    def process_game_spin(self, user_id):
        """Spin — no daily limit, costs 1 pass. Wheel stops on correct reward."""
        try:
            user_id = int(user_id)
            ...
            return {
                ...
                'segment_index': selected,  # HTML wheel stops here
    ...
    def process_game_guess(self, user_id, guess, bet):
        """Number guess — no daily limit, costs 1 pass."""
        try:
            user_id = int(user_id)
            today = datetime.now().date().isoformat()
            state = self.get_game_state(user_id, today)
    ...
    def process_game_coin(self, user_id, choice, bet):
        """Coin flip — no daily limit, costs 1 pass."""
        try:
            user_id = int(user_id)
    ...
    def process_game_scratch(self, user_id):
        """
        Scratch card — no daily limit, costs 1 pass.
        UPDATED rewards: 0.10=60%, 0.50=25%, 1=10%, 2=4%, 5=1%
        """
        try:
            user_id = int(user_id)
            ...
            # UPDATED: correct probability distribution
            roll = random.random()
            if roll < 0.60:
                reward = 0.10     # 60%
            elif roll < 0.85:
