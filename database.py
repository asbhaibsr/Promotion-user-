# ===== database.py (COMPLETE FIXED VERSION - WITH ALL GAME FUNCTIONS) =====
...
class Database:
    def __init__(self, config):
        ...
        try:
            ...
            self.game_states = self.db['game_states']  # NEW: for game state tracking
    ...
    def _create_indexes(self):
        try:
            ...
            self.daily_claims.create_index(
                [('user_id', ASCENDING), ('ad_id', ASCENDING), ('date', ASCENDING)],
                unique=True
            )
            ...
            if self.ads.count_documents({}) == 0:
                self.ads.insert_many([
                    {'id': 1, 'title': 'Install App & Earn', 'reward': 2.0,
                     ...
                     'meta': '⏱️ 2 min • 1.2k completed', 'icon': '📱', 'order': 1},
                    {'id': 2, 'title': 'Watch Video', 'reward': 0.5,
                     'link': 'https://t.me/+8SdeM5gBihoxZjU1',
                     'meta': '⏱️ 30 sec • 3.4k completed', 'icon': '🎬', 'order': 2},
                    {'id': 3, 'title': 'Join Channel', 'reward': 1.0,
                     'link': 'https://t.me/+8SdeM5gBihoxZjU1',
                     'meta': '⏱️ 1 min • 5.6k completed', 'icon': '📢', 'order': 3}
                ])
    ...
    def record_search(self, user_id):
        ...
        try:
            ...
            last_search = self.search_logs.find_one(
                {'user_id': user_id}, sort=[('timestamp', DESCENDING)]
            )
            if last_search:
            ...
            self.search_logs.insert_one({
                'user_id': user_id, 'timestamp': now.isoformat(), 'date': today
            })
            ...
            self.users.update_one(
                {'user_id': user_id},
                {'$inc': {'total_searches': 1}, '$set': {'last_active': now.isoformat()}}
            )
            ...
            for ref in active_refs:
                try:
                    ...
                    today_search = self.daily_searches.find_one(
                        {'user_id': referred_id, 'date': today}
                    )
                    if today_search:
    ...
    def mark_channel_join(self, user_id, channel_id):
        try:
            ...
            self.users.update_one(
                {'user_id': user_id}, {'$set': {'channel_joined': True}}
            )
            self.user_cache.pop(f"user_{user_id}", None)
            self.add_live_activity(
                'bonus', user_id, self.config.CHANNEL_JOIN_BONUS,
    ...
    def update_ad(self, ad_id, title, reward, link, meta, icon=None):
        try:
            update_data = {
                'title': title, 'reward': float(reward),
                'link': link, 'meta': meta
            }
            if icon:
                update_data['icon'] = icon
            self.ads.update_one(
                {'id': int(ad_id)}, {'$set': update_data}, upsert=True
            )
            return True
        except Exception as e:
    ...
    def get_user_claimed_ads(self, user_id, date=None):
        try:
            ...
            claims = list(self.daily_claims.find(
                {'user_id': user_id, 'date': date}, {'ad_id': 1, '_id': 0}
            ))
    ...
    def claim_ad(self, user_id, ad_id, reward):
        try:
            ...
            existing = self.daily_claims.find_one(
                {'user_id': user_id, 'ad_id': ad_id, 'date': today}
            )
            if existing:
                return False
            ...
            self.daily_claims.insert_one({
                'user_id': user_id, 'ad_id': ad_id, 'date': today,
                'reward': float(reward), 'timestamp': datetime.now().isoformat()
            })
    ...
    def approve_withdrawal(self, withdrawal_id, admin_id):
        try:
            ...
            self.add_transaction(
                withdrawal['user_id'], 'withdrawal_approved',
                -withdrawal['amount'],
                f"Withdrawal approved #{withdrawal_id[-8:]}"
            )
            self.add_live_activity(
                'withdraw', withdrawal['user_id'], withdrawal['amount'],
    ...
    # ============================================================
    # ========== GAME FUNCTIONS (NEW - MAIN FIX) ==========
    # ============================================================

    DAILY_GAME_LIMIT = 25.0
    SPIN_COST = 1.0
    SPIN_SEGMENTS = [
    ...
    SPIN_WEIGHTS = [3, 4, 5, 2, 6, 1, 3, 4]

    def get_game_state(self, user_id, date=None):
        """Get or create game state for user today"""
        try:
            user_id = int(user_id)
            if not date:
                date = datetime.now().date().isoformat()
            ...
            return {
                'spin_left': 5, 'guess_left': 3,
                'coin_left': 5, 'scratch_left': 2,
                'today_game_earned': 0.0, 'wins': 0, 'win_streak': 0
            }

    def _update_game_state(self, user_id, update_fields):
        """Update game state fields"""
        today = datetime.now().date().isoformat()
        self.game_states.update_one(
            {'user_id': int(user_id), 'date': today},
            {'$set': update_fields},
            upsert=True
        )

    def add_game_earning(self, user_id, amount, game_type='game', description='Game reward'):
        """Add game earning to user balance with daily limit check"""
        try:
            user_id = int(user_id)
            amount = float(amount)
            today = datetime.now().date().isoformat()
            ...
            remaining_limit = self.DAILY_GAME_LIMIT - already_earned

            if remaining_limit <= 0:
                return {
                    'success': False,
                    'message': 'Daily game limit of ₹25 reached!',
                    'earned': 0,
                    'limit_reached': True
                }

            actual_earn = min(amount, remaining_limit)

            # Add to balance
            self.add_balance(user_id, actual_earn, description)

            # Update game state
            new_earned = already_earned + actual_earn
            self.game_states.update_one(
                {'user_id': user_id, 'date': today},
                {'$set': {
                    'today_game_earned': new_earned,
                },
                 '$inc': {'wins': 1}},
                upsert=True
            )
            ...
            self.users.update_one(
                {'user_id': user_id},
                {'$inc': {'games_won': 1, 'total_game_earned': actual_earn}}
            )
            self.user_cache.pop(f"user_{user_id}", None)

            # Live activity
            self.add_live_activity('game', user_id, actual_earn, f"won ₹{actual_earn:.2f} in {game_type}")

            logger.info(f"Game earning: user {user_id} earned ₹{actual_earn} in {game_type}")
            return {
                'success': True,
                'earned': actual_earn,
                'today_total': new_earned,
                'remaining_limit': self.DAILY_GAME_LIMIT - new_earned,
                'limit_reached': new_earned >= self.DAILY_GAME_LIMIT
            }
        except Exception as e:
    ...
    def process_game_spin(self, user_id):
        ...
        try:
            ...
            if state.get('today_game_earned', 0) >= self.DAILY_GAME_LIMIT:
                return {'success': False, 'message': 'Daily game limit ₹25 poori ho gayi!'}

            # Deduct spin cost
            deduct_result = self.deduct_game_balance(user_id, self.SPIN_COST, 'spin')
            if not deduct_result.get('success'):
                return deduct_result

            # Choose reward by weighted random
            ...
            return {
                ...
                'spin_left': new_spin_left,
                'today_earned': earn_result.get('today_total', 0),
                'limit_reached': earn_result.get('limit_reached', False)
            }
        except Exception as e:
        ...
        try:
            ...
            if is_correct:
                ...
                earn_result = self.add_game_earning(
                    user_id, reward, 'guess', f"Guess correct! {reward_label if False else ''}"
                )
                result['reward'] = earn_result.get('earned', 0)
                result['today_earned'] = earn_result.get('today_total', 0)
                result['limit_reached'] = earn_result.get('limit_reached', False)

                # Start new game, reduce tries
                new_guess_left = max(0, state.get('guess_left', 3) - 1)
                self.game_states.update_one(
                    {'user_id': user_id, 'date': today},
                    {'$set': {
                        'guess_left': new_guess_left,
                        'guess_secret': random.randint(1, 10),
                        'guess_attempts_used': 0
                    }}
                )
    ...
    def process_game_scratch(self, user_id):
        ...
        try:
            ...
            # Generate reward
            roll = random.random()
            if roll < 0.4: reward = 0.1
            elif roll < 0.65: reward = 0.25
            elif roll < 0.8: reward = 0.5
            elif roll < 0.9: reward = 1.0
            elif roll < 0.96: reward = 2.0
            elif roll < 0.99: reward = 5.0
            else: reward = 10.0
