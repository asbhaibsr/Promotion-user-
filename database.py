# ===== database.py (FULLY UPDATED - ALL FIXES) =====
...
class Database:
    def __init__(self, config):
        ...
        try:
            ...
            self.monthly_refs = self.db['monthly_refs']  # NEW: track monthly active refs
    ...
    def _create_indexes(self):
        try:
            ...
            self.monthly_refs.create_index([('referrer_id', ASCENDING), ('year_month', ASCENDING)], unique=True)  # NEW
    ...
    # ============================================================
    # SPIN SEGMENTS & WEIGHTS (FIXED PROBABILITIES)
    # ============================================================
    SPIN_SEGMENTS = [
        {'label': '₹5',    'value': 5.0},   # index 0 — weight 1 (1%)
        {'label': '₹0.50', 'value': 0.50},  # index 1 — weight 6 (6%)
        {'label': '₹0.10', 'value': 0.10},  # index 2 — weight 15 (15%)
        {'label': '₹1',    'value': 1.0},   # index 3 — weight 3 (3%)
        {'label': '₹0.10', 'value': 0.10},  # index 4 — weight 15 (15%)
        {'label': '₹0.50', 'value': 0.50},  # index 5 — weight 6 (6%)
        {'label': '₹0.25', 'value': 0.25},  # index 6 — weight 10 (10%)
        {'label': '₹2',    'value': 2.0},   # index 7 — weight 1 (1%)
        {'label': '₹0.10', 'value': 0.10},  # index 8 — weight 15 (15%)
        {'label': '₹0.50', 'value': 0.50},  # index 9 — weight 6 (6%)
        {'label': '₹0.10', 'value': 0.10},  # index 10 — weight 15 (15%)
        {'label': '₹1',    'value': 1.0},   # index 11 — weight 3 (3%)
        {'label': '₹0.10', 'value': 0.10},  # index 12 — weight 15 (15%)
        {'label': '₹0.25', 'value': 0.25},  # index 13 — weight 10 (10%)
        {'label': '₹0.10', 'value': 0.10},  # index 14 — weight 15 (15%)
        {'label': '₹0.50', 'value': 0.50},  # index 15 — weight 6 (6%)
    ]
    SPIN_WEIGHTS = [1, 6, 15, 3, 15, 6, 10, 1, 15, 6, 15, 3, 15, 10, 15, 6]  # Total: 152
    ...
    def activate_referral_by_log_channel(self, referred_id):
        """Called when new user joins via log channel message."""
        try:
            ...
            # ✅ NEW: Track monthly active ref
            self._add_monthly_active_ref(referrer_id, referred_id, now)
    ...
    # ========== MONTHLY ACTIVE REFS TRACKING (NEW) ==========
    def _add_monthly_active_ref(self, referrer_id, referred_id, activation_date_iso):
        """Track active referral for monthly count."""
        try:
            activation_date = datetime.fromisoformat(activation_date_iso)
            year_month = activation_date.strftime("%Y-%m")
            
            self.monthly_refs.update_one(
                {'referrer_id': int(referrer_id), 'year_month': year_month},
                {'$inc': {'count': 1}, '$push': {'referred_ids': int(referred_id)}},
                upsert=True
            )
            logger.info(f"Monthly ref tracked: {referrer_id} for {year_month}")
        except Exception as e:
            logger.error(f"Error adding monthly active ref: {e}")

    def get_month_active_refs(self, user_id):
        """Count active referrals activated in current calendar month."""
        try:
            user_id = int(user_id)
            now = datetime.now()
            year_month = now.strftime("%Y-%m")
            
            record = self.monthly_refs.find_one({'referrer_id': user_id, 'year_month': year_month})
            count = record.get('count', 0) if record else 0
            
            # Also check legacy method for backward compatibility
            if count == 0:
                ...
                if count > 0:
                    self.monthly_refs.update_one(
                        {'referrer_id': user_id, 'year_month': year_month},
                        {'$set': {'count': count}},
                        upsert=True
                    )
            return count
    ...
    def record_daily_search(self, referred_user_id):
        """Called when a referred user searches movie in group."""
        try:
    ...
    def claim_day_bonus(self, user_id, date_str):
        """UPDATED: streak bonus ₹0.05/day, max ₹0.50 extra"""
        try:
            user_id = int(user_id)
            user = self.get_user(user_id)
            if not user:
            ...
            streak_bonus = min(streak * 0.05, 0.50)   # UPDATED: 0.05/day max 0.50
    ...
    def update_ad(self, ad_id, title, reward, link, meta, icon=None, claim_code=None, timer_seconds=0):
        """Update ad. timer_seconds = silent wait before claim allowed."""
        try:
            update_data = {
                'title': title,
                'reward': float(reward),
    ...
    def process_withdrawal(self, user_id, amount, method, details):
        """UPDATED: min ₹20, requires 20 active refs this month."""
        try:
            user_id = int(user_id)
            amount = float(amount)
            user = self.get_user(user_id)
            ...
            if amount < 20.0:
                return {'success': False, 'message': 'Minimum withdrawal is ₹20'}

            month_refs = self.get_month_active_refs(user_id)
            if month_refs < 20:
                return {
                    'success': False,
                    'message': f'Is mahine sirf {month_refs}/20 active refs hain. Withdraw ke liye is mahine 20 active referrals zaroori hain!'
                }
            ...
            withdrawal = {
                ...
                'month_active_refs': month_refs
            }
            result = self.withdrawals.insert_one(withdrawal)
            self.add_transaction(user_id, 'withdrawal_request', -amount,
                                 f"Withdrawal #{str(result.inserted_id)[-6:]}")
            self.add_live_activity('withdraw_request', user_id, amount,
    ...
    # === SPIN GAME (FIXED — segment_index return) ===
    def process_game_spin(self, user_id):
        """FIXED: segment_index sent so frontend wheel stops at EXACT correct prize."""
        try:
            user_id = int(user_id)
            user = self.get_user(user_id)
            if not user or user.get('passes', 0) <= 0:
            ...
            return {
                ...
                'segment_index': selected,   # CRITICAL: frontend uses this to stop wheel
    ...
    # === DICE GAME (NEW — replaces scratch) ===
    def process_game_dice(self, user_id, choice, bet=1):
        """
        Dice Roll game:
        - User picks 1-6
        - Dice rolls randomly
        - Win if choice == rolled
        - 60% house edge: 40% chance to win
        - Reward: 5x bet
        """
        try:
            user_id = int(user_id)
            ...
            if choice < 1 or choice > 6:
                return {'success': False, 'message': 'Invalid choice. Pick 1-6.'}
            ...
            self.add_transaction(user_id, 'game_bet', -bet, "Dice roll bet")

            if not self.deduct_pass(user_id):
                return {'success': False, 'message': 'Pass deduct nahi hua'}

            rolled = random.randint(1, 6)
            is_match = (rolled == choice)
            
            # 60% house edge - even if match, only 40% chance to win
            if is_match:
                house_random = random.random()
                if house_random < 0.4:  # 40% chance to win
                    reward = bet * 5  # 5x reward
                    earn_result = self.add_game_earning(user_id, reward, 'dice', f"Dice Roll win ₹{reward}")
                    self.user_cache.pop(f"user_{user_id}", None)
                    return {
                        'success': True,
                        'won': True,
    ...
    # === CARD BATTLE GAME (NEW) ===
    def process_game_card(self, user_id, choice, bet=1):
        """
        Card Battle game:
        - User picks suit (hearts, diamonds, clubs, spades)
        - Computer picks random suit
        - Card ranks: 1-13 (Ace=1, King=13)
        - Win if rank higher AND suit beats? Simplified: higher rank wins
        - Tie = money back
        - 60% house edge overall
        """
        try:
            user_id = int(user_id)
            suits = ['hearts', 'diamonds', 'clubs', 'spades']
            
            if choice not in suits:
            ...
            # Deduct bet amount
            self.users.update_one({'user_id': user_id}, {'$inc': {'balance': -bet}})
            self.add_transaction(user_id, 'game_bet', -bet, "Card battle bet")
            ...
            # Player card
            player_suit = choice
            player_rank = random.randint(1, 13)
            
            # Computer card
            computer_suit = random.choice(suits)
            computer_rank = random.randint(1, 13)
            ...
            # House edge: even wins, only 40% actually pay
            if result == 'win':
                house_random = random.random()
                if house_random < 0.4:  # 40% chance to actually win
                    reward = bet * 2  # 2x reward
                    earn_result = self.add_game_earning(user_id, reward, 'card', f"Card Battle win ₹{reward}")
                    self.user_cache.pop(f"user_{user_id}", None)
                    return {
                        'success': True,
                        'result': 'win',
                        ...
                        'computer_suit': computer_suit,
                        'computer_rank': computer_rank,
                        'reward': earn_result.get('earned', reward),
                        'today_earned': earn_result.get('today_total', 0)
                    }
                else:
                    result = 'lose'  # House takes it
            
            if result == 'tie':
                # Refund bet
                self.users.update_one({'user_id': user_id}, {'$inc': {'balance': bet}})
                self.add_transaction(user_id, 'game_refund', bet, "Card battle tie refund")
                self.user_cache.pop(f"user_{user_id}", None)
                return {
                    'success': True,
                    'result': 'tie',
            ...
            logger.error(f"Error processing card battle: {e}")
            return {'success': False, 'message': str(e)}
    ...
    def process_game_guess(self, user_id, guess, bet):
        try:
            ...
            # Deduct bet amount
            self.users.update_one({'user_id': user_id}, {'$inc': {'balance': -bet}})
            self.add_transaction(user_id, 'game_bet', -bet, "Guess game bet")
    ...
    def process_game_coin(self, user_id, choice, bet):
        try:
            ...
            # Deduct bet amount
            self.users.update_one({'user_id': user_id}, {'$inc': {'balance': -bet}})
            self.add_transaction(user_id, 'game_bet', -bet, "Coin flip bet")
            ...
            if won:
                # 60% house edge - only 40% of wins actually pay
                house_random = random.random()
                if house_random < 0.4:  # 40% chance to actually win
                    reward = bet * 2
                    ...
                    result['won'] = False  # House takes it
