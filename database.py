    ...
    def get_live_activity(self, limit=20):
        ...
        try:
            ...
            for act in activities:
                ...
                if not display_text:
                    if act['type'] == 'join':
                        display_text = f"joined the bot"
                    elif act['type'] == 'withdraw':
                        display_text = f"withdrew ₹{act['amount']}"
                    elif act['type'] == 'bonus':
                        display_text = f"claimed ₹{act['amount']} bonus"
                    elif act['type'] == 'mission':
                        display_text = f"completed missions"
                    elif act['type'] == 'referral':
                        display_text = f"got a new referral"
                    else:
